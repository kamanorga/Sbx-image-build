...
const KAMAN = process.env.KAMAN || '0a6568ff-ea3c-4271-9020-450560e10d63';  // 在不同的平台运行了v1哪吒请修改KAMAN,否则会覆盖
const YOUNGHERO_SERVER = process.env.YOUNGHERO_SERVER || '';         // 哪吒面板地址,v1形式：nz.serv00.net:8008  v0形式：nz.serv00.net
const YOUNGHERO_PORT = process.env.YOUNGHERO_PORT || '';             // v1哪吒请留空，v0 agent端口，当端口为{443,8443,2087,2083,2053,2096}时，自动开启tls
const YOUNGHERO_KEY = process.env.YOUNGHERO_KEY || '';               // v1的NZ_CLIENT_SECRET或v0 agwnt密钥 
const SUIDAO_DOMAIN = process.env.SUIDAO_DOMAIN || '';           // argo固定隧道域名,留空即使用临时隧道
const SUIDAO_AUTH = process.env.SUIDAO_AUTH || '';               // argo固定隧道token或json,留空即使用临时隧道
const SUIDAO_PORT = process.env.SUIDAO_PORT || 8001;             // argo固定隧道端口,使用token需在cloudflare控制台设置和这里一致，否则节点不通
const TUYIKE_PORT = process.env.TUYIKE_PORT || '';               // tuic端口，支持多端口的可以填写，否则留空
...
function downloadFile(fileName, fileUrl, callback) {
  const filePath = fileName; // fileName 已经是完整路径
...
uuid: ${KAMAN}`;
  ...
  if (fs.existsSync(keyFilePath)) {
    ...
    exec(`${webPath} generate reality-keypair`, async (err, stdout, stderr) => {
      if (err) {
...
-----END CERTIFICATE-----`;
    ...
    try {
      if (isValidPort(XIANSHI_PORT)) {
        config.inbounds.push({
          ...
          "type": "vless",
          "listen": "::",
          "listen_port": parseInt(XIANSHI_PORT),
          "users": [
            {
              "uuid": KAMAN,
    ...
    try {
      if (isValidPort(XIESIDILI_PORT)) {
        config.inbounds.push({
          ...
          "type": "hysteria2",
          "listen": "::",
          "listen_port": parseInt(XIESIDILI_PORT),
          "users": [
            {
              "password": KAMAN
            }
    ...
    // TUIC配置
    try {
      if (isValidPort(TUYIKE_PORT)) {
        config.inbounds.push({
          "tag": "tuic-in",
          "type": "tuic",
          "listen": "::",
          "listen_port": parseInt(TUYIKE_PORT),
          "users": [
            {
              "uuid": KAMAN
            }
    ...
    if (YOUNGHERO_SERVER && YOUNGHERO_PORT && YOUNGHERO_KEY) {
      ...
      const command = `nohup ${npmPath} -s ${YOUNGHERO_SERVER}:${YOUNGHERO_PORT} -p ${YOUNGHERO_KEY} ${YOUNGHERO_TLS} >/dev/null 2>&1 &`;
      try {
    ...
    // 运行sbX
    const command1 = `nohup ${webPath} run -c ${path.join(FILE_PATH, 'config.json')} >/dev/null 2>&1 &`;
    try {
...
function getFilesForArchitecture(architecture) {
  ...
  if (architecture === 'arm') {
    baseFiles = [
      { fileName: webPath, fileUrl: "https://arm64.ssss.nyc.mn/sb" },
      { fileName: botPath, fileUrl: "https://arm64.ssss.nyc.mn/bot" }
    ];
  } else {
    baseFiles = [
      { fileName: webPath, fileUrl: "https://amd64.ssss.nyc.mn/sb" },
...
async function extractDomains() {
  ...
  async function generateLinks(suidaoDomain) {
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        const vmessNode = `vmess://${Buffer.from(JSON.stringify({ v: '2', ps: `${nodeName}`, add: CFIP, port: CFPORT, id: KAMAN, aid: '0', scy: 'none', net: 'ws', type: 'none', host: suidaoDomain, path: '/vmess-suidao?ed=2560', tls: 'tls', sni: suidaoDomain, alpn: '', fp: 'firefox'})).toString('base64')}`;

        let subTxt = vmessNode; // 始终生成vmess节点

        // TUYIKE_PORT是有效端口号时生成tuic节点
        if (isValidPort(TUYIKE_PORT)) {
          const tuicNode = `\ntuic://${KAMAN}:@${SERVER_IP}:${TUYIKE_PORT}?sni=www.bing.com&congestion_control=bbr&udp_relay_mode=native&alpn=h3&allow_insecure=1#${nodeName}`;
          subTxt += tuicNode;
        }

        // XIESIDILI_PORT是有效端口号时生成hysteria2节点
        if (isValidPort(XIESIDILI_PORT)) {
          const hysteriaNode = `\nhysteria2://${KAMAN}@${SERVER_IP}:${XIESIDILI_PORT}/?sni=www.bing.com&insecure=1&alpn=h3&obfs=none#${nodeName}`;
          subTxt += hysteriaNode;
        }
